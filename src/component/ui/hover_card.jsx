...
function HoverExpand({
  ...
  return (
    <div className="relative">
      <div className="mx-auto flex w-fit gap-1 rounded-md pb-10 pt-5 md:gap-2">
        {images.slice(0, maxThumbnails).map((imageUrl, i) => (
          <div
            key={`image-container-${i}`}
            className={cn(
              "group relative h-80 overflow-hidden rounded-2xl transition-all duration-300",
              selectedIndex === i ? "w-70" : "w-6 sm:w-10 md:w-20 xl:w-30"
            )}
